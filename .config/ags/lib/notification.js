...
const NotificationIcon = (notifObject) => {
  ...
  let icon = "NO_ICON";
  if (lookUpIcon(notifObject.appIcon)) icon = notifObject.appIcon;
  if (lookUpIcon(notifObject.appEntry)) icon = notifObject.appEntry;
...
export default ({
  ...
  if (
    ...
    notifTime = "Yesterday";
  else notifTime = messageTime.format("%d/%m");
  const notifText = Box({
    valign: Gtk.Align.CENTER,
    vertical: true,
    hexpand: true,
      ...
      self
          ...
          (self) => {
            ...
            if (initDirVertical == 0 && offset_x > MOVE_THRESHOLD) {
              if (initDirX < 0)
                ...
                self.setCss(`
                            margin-left:   ${Number(
                              offset_x + startMargin - MOVE_THRESHOLD
                            )}px;
                            margin-right: -${Number(
                              offset_x + startMargin - MOVE_THRESHOLD
                            )}px;
        ...
        .hook(
          ...
          (self) => {
            ...
            if (
              Math.abs(offset_h) > DRAG_CONFIRM_THRESHOLD &&
              offset_h * initDirX > 0
            ) {
              if (offset_h > 0) {
              ...
              if (widget.window)
                widget.window.set_cursor(
                  Gdk.Cursor.new_from_name(display, "grab")
                );
