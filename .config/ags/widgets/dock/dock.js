...
const pinnedApps = ["firefox", "org.gnome.Nautilus"];
        ...
        (box) => {
          for (let i = 0; i < Hyprland.clients.length; i++) {
            ...
            for (const appName of pinnedApps) {
              if (appClass.includes(appName.toLowerCase())) return null;
            }
...
export default () => {
  const dockContent = Box({
    ...
    children: [PinnedApps(), DockSeparator(), Taskbar()],
  });
